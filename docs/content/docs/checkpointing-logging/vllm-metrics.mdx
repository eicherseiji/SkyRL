...
SkyRL ships a Grafana dashboard covering throughput, latency, engine queue
depth, and KV-cache metrics. It is built from
[vllm_dashboard.py](skyrl/train/utils/vllm_dashboard.py), which keeps the panel
definitions and PromQL in one place; the generated JSON is committed next to it
at `skyrl/train/utils/vllm_dashboard.json`.

Import the committed JSON directly (Dashboards → New → Import), or regenerate it
against a different Prometheus data source UID:

```bash
uv run --isolated --extra dev python -m skyrl.train.utils.vllm_dashboard > vllm_dashboard.json
```

The default data source UID matches the reference dashboard; pass a different
one by editing `DEFAULT_DATASOURCE_UID` or calling `build_dashboard(uid)`.
